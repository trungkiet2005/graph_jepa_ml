...
def build_cfg(dataset_name):
    ...
    cfg.train = types.SimpleNamespace(
        ...
        vicreg_weight=0.0,   # VICReg std+cov on GNN patch embeddings (see apply_sota_push_overrides)
    )
...
def apply_sota_push_overrides(cfg, benchmark_key: str):
    ...
    Also enables VICReg (Bardes et al., 2021) auxiliary loss on graph-wise mean patch embeddings."""
    ...
    # Wider trunk + stronger linear readout (10-fold linear probe)
    cfg.model.hidden_size = max(getattr(cfg.model, 'hidden_size', 512), 768)
    cfg.train.logreg_C = 4.0
    cfg.train.logreg_class_weight = 'balanced'
    cfg.train.logreg_solver = 'lbfgs'
    ...
    # VICReg (Bardes et al., 2021): variance + covariance on batch of graph-wise patch means
    # stabilizes JEPA trunk; std+cov only (no invariance term — prediction loss already aligns views).
    cfg.train.vicreg_weight = 0.06

    if benchmark_key == 'ZINC':
        cfg.model.nlayer_gnn = max(cfg.model.nlayer_gnn, 3)
        cfg.train.epochs = max(cfg.train.epochs, 72)
        cfg.train.scale_linear_eval = True
        cfg.train.ridge_alpha = 0.2
    ...
    if benchmark_key == 'DD':
        cfg.train.epochs = max(cfg.train.epochs, 40)
        cfg.model.nlayer_gnn = max(cfg.model.nlayer_gnn, 3)

    if benchmark_key == 'EXP':
        cfg.train.epochs = max(cfg.train.epochs, 55)

    if benchmark_key == 'IMDB-BINARY':
        cfg.train.epochs = max(cfg.train.epochs, 64)
        cfg.train.lr_patience = max(cfg.train.lr_patience, 28)

    if benchmark_key == 'IMDB-MULTI':
        cfg.train.epochs = max(cfg.train.epochs, 48)
        cfg.train.lr_patience = max(cfg.train.lr_patience, 24)

    if benchmark_key in ('REDDIT-BINARY', 'REDDIT-MULTI-5K'):
        cfg.train.epochs = max(cfg.train.epochs, 28)
        cfg.train.lr_patience = max(cfg.train.lr_patience, 28)
...
class GraphJepa(nn.Module):
    ...
    def forward(self, data):
        ...
        vic_aux = subgraph_x.new_zeros(())
        if self.training and self.vicreg_weight and self.vicreg_weight > 0:
            z_g = _patches_to_graph_mean_repr(subgraph_x, data.call_n_patches, data.y.device)
            vic_aux = vicreg_std_cov_loss(z_g)
        ...
        return target_x, target_y, vic_aux
...
class GraphHMSJepa(nn.Module):
    ...
    def forward(self, data):
        ...
        vic_aux = subgraph_x_L0.new_zeros(())
        if self.training and self.vicreg_weight and self.vicreg_weight > 0:
            z_g = _patches_to_graph_mean_repr(subgraph_x_L0, data.call_n_patches, device)
            vic_aux = vicreg_std_cov_loss(z_g)
        return (tgt_x_L0, pred_L0), (tgt_x_L1, pred_L1), (tgt_x_L2, pred_L2), vic_aux
    ...
    if cfg.metis.n_patches > 0:
        if cfg.jepa.enable:
            common_kwargs = dict(
                ...
                vicreg_weight=getattr(cfg.train, 'vicreg_weight', 0.0),
            )
            use_hms = getattr(cfg.jepa, 'num_scales', 1) > 1
            if use_hms:
                return GraphHMSJepa(
...
def _per_graph_patch_counts(call_n_patches) -> list:
    """Normalize PyG-batched `call_n_patches` into a list of ints (patches per graph)."""
    if torch.is_tensor(call_n_patches):
        return [int(x) for x in call_n_patches.flatten().tolist()]
    out = []
    for item in call_n_patches:
...
def _patches_to_graph_mean_repr(subgraph_x: Tensor, call_n_patches, device) -> Tensor:
    """Mean-pool patch embeddings per graph → (B, D) for VICReg."""
    counts = _per_graph_patch_counts(call_n_patches)
    if not counts:
        return subgraph_x.mean(dim=0, keepdim=True)
    if sum(counts) != subgraph_x.size(0):
        # Fallback: single graph or unexpected collation
        return subgraph_x.mean(dim=0, keepdim=True)
    chunks = torch.split(subgraph_x, counts, dim=0)
    return torch.stack([c.mean(dim=0) for c in chunks], dim=0)


def vicreg_std_cov_loss(z: Tensor, eps: float = 1e-4) -> Tensor:
    """Standard deviation + off-diagonal covariance penalty (VICReg-style, no invariance term)."""
    b, d = z.shape
...
def _compute_loss(model, data, criterion, criterion_type):
    ...
    if isinstance(_unwrap_compile(model), GraphHMSJepa):
        (tx0, ty0), (tx1, ty1), (tx2, ty2), vic_term = model(data)
        if criterion_type == 0:
        ...
        target_x, target_y, vic_term = model(data)
        if criterion_type == 0:
            ...
            sys.exit()
        num_t = len(target_y)
    vw = getattr(_unwrap_compile(model), 'vicreg_weight', 0.0)
    if vw and vw > 0:
        loss = loss + vw * vic_term
    return loss, num_t
...
def _make_logistic_regressor(cfg):
    """Sklearn head for TU graph classification; tuned hyperparameters when SOTA sets logreg_*."""
    if not any(getattr(cfg.train, a, None) is not None for a in ('logreg_C', 'logreg_class_weight', 'logreg_solver')):
        return LogisticRegression(max_iter=50000)
    return LogisticRegression(
        max_iter=100000,
        C=getattr(cfg.train, 'logreg_C', 1.0),
        class_weight=getattr(cfg.train, 'logreg_class_weight', None),
        solver=getattr(cfg.train, 'logreg_solver', 'lbfgs'),
        random_state=0,
    )
...
def run_k_fold(cfg, create_dataset_fn, create_model_fn, train_fn, test_fn, evaluator=None, k=10):
    ...
    for run_idx in range(cfg.train.runs):
        ...
        for fold, (train_idx, test_idx) in enumerate(zip(*k_fold_indices)):
            ...
            lin_model = _make_logistic_regressor(cfg)
            lin_model.fit(X_train, y_train)
            lin_predictions = lin_model.predict(X_test)
...
def main():
    ...
    SOTA_PUSH = True        # True = HMS-JEPA + AdamW/wd/dropout + wider hidden + LapPE + VICReg + tuned linear probe (see apply_sota_push_overrides)
